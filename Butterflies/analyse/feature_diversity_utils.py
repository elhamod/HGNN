...
def deconv_orth_dist(kernel, stride = 2, padding = 1):
    return deconv_orth_dist_2(kernel, kernel, stride, padding)

def deconv_orth_dist_2(kernel1, kernel2, stride = 2, padding = 1):
    [o_c, i_c, w, h] = kernel1.shape
    [o_c_2, i_c_2, w_2, h_2] = kernel2.shape
    assert o_c == o_c_2
    assert i_c == i_c_2
    assert w == w_2
    assert h == h_2
...
def orth_dist(mat, stride=None):
    return orth_dist_2(mat, mat, stride)

def orth_dist_2(mat1, mat2, stride=None):
    assert mat1.shape[0] == mat1.shape[0]
    ...
    return torch.norm( torch.t(mat1)@mat2 - torch.eye(mat1.shape[1]).cuda())
